...
import { UserRound, Check, X, AlertTriangle, FileText } from 'lucide-react';
...
function ApproveReject({ onApprove, onReject, state }: {
  ...
  if (state === 'approved')
    return <span className="flex items-center gap-1 text-xs text-muted-foreground"><Check className="h-3 w-3" />Approved</span>;
  if (state === 'rejected')
    return <span className="flex items-center gap-1 text-xs text-muted-foreground"><X className="h-3 w-3" />Rejected</span>;
  return (
    <div className="flex items-center gap-1.5">
      <button onClick={onApprove} className="flex items-center gap-1 rounded-md bg-muted px-2 py-1 text-xs font-medium text-foreground hover:bg-muted/80 transition-colors">
        <Check className="h-3 w-3" /> Approve
      </button>
      <button onClick={onReject} className="flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
...
function ReviewSection() {
  ...
  const items = [
    { kind: 'citation', title: 'Verify citation accuracy', meta: 'IPCC 2023 report p. 12', summary: 'The agent cited Table 3 from the IPCC AR6 Synthesis; confirm page and table reference before locking.' },
    { kind: 'quote', title: 'Confirm highlighted quote', meta: 'Policy Brief §3.1', summary: 'A direct quote from the EU Climate Law Policy Brief was used verbatim. Confirm this is an accurate pull.' },
    { kind: 'section', title: 'Approve section for export', meta: 'Writing · Section 2', summary: 'Section 2 is ready for export. Review the final paragraph before confirming.' },
  ];
      ...
      {items.map((item, i) => (
        <div key={i} className="rounded-lg border border-border overflow-hidden">
          <div className="flex items-start gap-3 p-3">
            <div className="mt-0.5 w-[3px] self-stretch rounded-full shrink-0 bg-border" />
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-2 mb-1">
                <span className="text-xs font-medium text-foreground">{item.title}</span>
                <span className="shrink-0 text-[10px] text-muted-foreground/60 capitalize">{item.kind}</span>
              </div>
  ...
  return (
    <div className="space-y-3">
      {NOTES_GROUPS.map((group) => (
        <div key={group.title}>
          <p className="mb-1.5 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">{group.title}</p>
          <div className="space-y-2">
...
function BibSection() {
  ...
  return (
    <div className="space-y-2">
      {entries.map((entry) => (
        <div key={entry.id} className={cn('group rounded-lg border p-3', entry.incomplete ? 'border-border' : 'border-border')}>
          {entry.incomplete && (
          ...
          <button
            ...
            className="mt-2 text-[10px] text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity"
          >
            Remove
          </button>
        </div>

...
export function Color() {
  return (
        <section className="mt-16">
          ...
          <ul className="aka-breakout mt-6 grid list-none grid-cols-2 gap-3 p-0 sm:grid-cols-3 lg:grid-cols-6">
            {SWATCHES.map((sw) => (
          ...
          <ul className="mt-3 flex list-none flex-wrap gap-2 p-0">
            {ACCENTS.map((a) => (
              <li
                key={a.name}
                className={`${card} flex items-center gap-2 rounded-full px-3 py-1.5 font-mono text-11 text-muted-foreground`}
              >
                <span

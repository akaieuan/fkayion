    ...
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 flex items-center gap-1">
      {!audioSrc && (
        <label className="p-2.5 rounded-full bg-black/40 backdrop-blur-md hover:bg-black/60 cursor-pointer transition-all group">
          <Upload className="h-4 w-4 text-white/75 group-hover:text-white/80 transition-colors" />
      ...
      <button
        ...
        className={`p-2.5 rounded-full backdrop-blur-md transition-all ${
          isSidebarOpen ? 'bg-white/20 text-white' : 'bg-black/40 hover:bg-black/60 text-white/75 hover:text-white/80'
        }`}
...
function MainContent() {
  return (
    <main className="fixed inset-0 bg-black">
      ...
      <Link
        href="/demo"
        className="fixed top-4 left-4 z-50 flex items-center gap-1.5 rounded-full bg-black/40 backdrop-blur-md px-3 py-1.5 text-xs text-white/75 hover:text-white/80 hover:bg-black/60 transition-all"
      >
